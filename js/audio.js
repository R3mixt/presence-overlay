...
/* ============ SYSTEM VOICE (TTS for non-narrated interface text) ============
   Voice quality varies wildly between platforms. Rather than taking the first
   English voice (often a legacy robotic one), rank everything available and
   prefer the modern neural voices: Edge "Natural", Chrome "Google" network
   voices, Apple's premium voices. Legacy desktop voices score last. */
...
function scoreVoice(v){
  ...
  if(/desktop|compact|espeak|david|sam|mark/i.test(n))s-=40;
  if(/zira|hazel|susan/i.test(n))s-=34; // legacy too, but the least grating of them
  if(/female|zira|hazel|aria|jenny|sonia|libby/i.test(n))s+=2; // the Presence reads closest to Suriel's own
  return s;
}
function pickVoice(){
  const vs=speechSynthesis.getVoices();
  if(!vs.length)return null;
  // even when only legacy voices exist, the best English one beats the default
  let best=null,bs=-1e4;
  for(const v of vs){const s=scoreVoice(v);if(s>bs){bs=s;best=v;}}
  return bs>-1e4?best:null;
}
if('speechSynthesis'in window){
  chosenVoice=pickVoice();
  speechSynthesis.onvoiceschanged=()=>{chosenVoice=pickVoice();};
}
// smooth out abbreviations and interface glyphs the synthesizer trips over
...
function utter(text,rate){
  ...
  u.rate=rate;u.pitch=1.0;u.volume=0.95;
  if(chosenVoice)u.voice=chosenVoice;
  return u;
}
function presenceVoice(text,interrupt){
  if(!voiceOn||!('speechSynthesis'in window))return;
  if(speaking||(audioEl&&!audioEl.paused))return; // never talk over narration
  try{
    if(interrupt!==false)speechSynthesis.cancel();
    speechSynthesis.speak(utter(speakable(text),1.02));
  }catch(e){}
}
...
/* ============ NARRATION ============ */
let audioEl=null,speaking=false;
const playBtn=document.getElementById('playBtn');
playBtn.onclick=()=>{
  initAudio();
  if(speaking||(audioEl&&!audioEl.paused)){stopAudio();return;}
  if(!currentNode)return;
  const url=audioMap[currentNode.id];
  if(url){
    audioEl=new Audio(url);audioEl.play();
    playBtn.classList.add('playing');document.getElementById('playLbl').textContent='Stop narration';
    audioEl.onended=stopAudio;
  }else{
    // read the record sentence by sentence — the pauses pace it like narration
    // instead of one long synthesized run-on
    const txt=speakable(document.getElementById('pBody').textContent);
    const parts=txt.match(/[^.!?]+[.!?]+["']?\s*/g)||[txt];
    speechSynthesis.cancel();
    speaking=true;
    playBtn.classList.add('playing');document.getElementById('playLbl').textContent='Stop narration';
    parts.forEach((p,i)=>{
      const u=utter(p.trim(),0.95);
      if(i===parts.length-1)u.onend=stopAudio;
